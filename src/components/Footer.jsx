...
import FormInput from './forms/FormInput'; // ✨ Import FormInput
...
function Footer({ theme = 'light' }) {
  ...
  return (
    <footer className={`${colors.bg} ${colors.text} z-50 transition-colors duration-300`}>
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pt-12 sm:pt-16 lg:pt-20 pb-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 xl:gap-20">
          ...
          <div className="lg:col-span-7 space-y-6 relative">
            ...
            {/* ✨ Newsletter Form with FormInput */}
            <form onSubmit={handleSubmit} className="newsletter-form mt-4 space-y-6 max-w-lg">
              <FormInput
                id="newsletter-email"
                name="email"
                type="email"
              ...
              <div className="flex gap-4 items-center pt-2">
                <button
                  ...
                  type="submit"
                  disabled={isSubmitting}
                  className={`subscribe-btn px-8 py-2.5 ${colors.button} text-xs tracking-[0.2em] uppercase font-medium transition-all duration-300 hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed`}
                >
                  {isSubmitting ? 'SUBSCRIBING...' : subscribed ? 'SUBSCRIBED!' : 'SUBSCRIBE'}
                </button>
                
                {subscribed && (
                  <div className="text-xs text-green-600 tracking-wider animate-fadeIn success-indicator">
                    ✓ Confirmed
                  </div>
                )}
              </div>
            ...
            {subscribed && (
              <div className={`absolute inset-0 flex items-center justify-center ${colors.successOverlay} z-10 rounded-lg`}>
                <div className="text-center">
                  <div className={`w-8 h-8 border-2 ${colors.successBorder} rounded-full flex items-center justify-center mx-auto mb-2 animate-bounce`}>
            ...
            <ul role="list" className="space-y-4">
              {navigation.support.map((item) => (
                <li key={item.name}>
                  <a
                    href={item.href}
                    className={`text-sm ${colors.textSecondary} ${colors.linkHover} transition-colors duration-300 block py-1 uppercase tracking-[0.1em]`}
                  >
                    {item.name}
                  </a>
                </li>
        ...
        {/* Bottom Section */}
        <div className={`mt-12 sm:mt-16 border-t ${colors.border} pt-8 sm:flex sm:items-center sm:justify-between`}>
          <p className={`text-sm ${colors.textSecondary} sm:order-1 uppercase tracking-[0.1em]`}>
            &copy; {currentYear} JGPNR.NG. All rights reserved.
          </p>
          
          {/* Social links */}
          <div className="flex space-x-6 mt-4 sm:mt-0">
            <a href="#" className={`${colors.icon} transition-colors duration-300`}>
